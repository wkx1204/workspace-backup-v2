...
## 2026-04-07：坦克大战复盘——subagent 写大文件根本不靠谱

**问题**：让 subagent 独立写一个 900+ 行的游戏 HTML，连续 4 个版本全部失败：
- 第一个版本：语法正确但游戏不工作
- 第二个版本：修到一半跑去画饼，越改越错
- 第三个版本：又修了 3 个版本，从 648 行 minify 到 57 行直接写坏
- 第四个版本：视觉重写 subagent 超时跑路，什么都没写

**根因**：
1. subagent 写大文件（900+行）能力不足，容易中途放弃或乱写
2. minify 步骤完全多余——写完正常代码再考虑压缩，一次性把 648 行压成 57 行必然出错
3. subagent 协作链太长、接口不清晰，没有版本控制，一个坏了接着修越修越烂
4. 自己（小白的第一个版本）其实是 937 行完整可运行的代码，只是被后续 subagent 污染了

**教训**：
- **大文件代码任务不外包**：900 行以上的完整功能代码必须自己写，不走 subagent
- **minify 是最后优化步骤，不是开发步骤**：先把功能跑通，再考虑压缩，一次压缩失败全毁
- **subagent 只做局部修改**：subagent 擅长修 bug、做小功能，不适合从零写完整大文件
- **中间版本要 git commit**：每次大改动 commit 一次，出问题可以回退
- **快速验证是金标准**：每写完一个功能立刻 Playwright 验证，不等"全写完再测"

**这次小白自己的版本验证结果**：
- 语法：✅ new Function() 验证通过
...
## 2026-04-17：梦境蒸馏知识（从 DREAMS.md 提炼）

### enabled ≠ allowed
- 配置开关打开 ≠ 实际有权限
- key 存在、配置正确，仍然可能被锁在门外
- 排查时：先验证 key 是否真正有权限，不要只看配置

### 先看本地，再查外网
- 老板电脑里有 Obsidian Vault，里面有数据
- 不要舍近求远去网上搜，本地文件优先
- 数据位置：~/Documents/Obsidian Vault/2-resource/建筑工程造价/

### 给窗户，不给镜子
- 用户要具体案例（真实造价编号、案例），不要给抽象指标（指数、参数）
- 区分：用户要的是"能用的结果"，不是"看起来专业的数据"

---

## 2026-04-17：MEMORY.md 只放老板原则和核心系统配置
- 项目信息 → Obsidian Vault github-research 目录
- 知识点/教训 → .learnings/LEARNINGS.md
- MEMORY.md 控制在 ≤3KB，只保留老板原则 + 系统配置
